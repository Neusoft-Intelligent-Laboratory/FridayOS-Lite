...
### ⚙️ skills/ —— 技能区
你反复让 Friday 做的同一类事，让它把步骤沉淀成 skill 文件，下次一句"老规矩"调用。**大脑出生就自带两个技能**（清理 inbox、每周回顾），都是照模板写的——之后你的每个好习惯都能这样沉淀。
...
| "把这个沉淀成 skill" | 按 system/模板/Skill模板.md 写一个新技能 |
...
大脑出生自带两个技能，Friday 搭建时存入 `skills/`：
...
1. 确认当前工作目录就是主人的 vault 根目录（本蓝图所在的文件夹）。
...
5. 把**第五节两个技能**分别写入 `skills/清理inbox.md`、`skills/每周回顾.md`（created 填今天）。
...
8. 把本蓝图移动到 `system/` 归档。
9. **⭐ 采访主人**（一次问完三个问题，别啰嗦）：① 怎么称呼你？② 这个大脑主要帮你管什么（工作/项目/学习/生活…）？③ 你喜欢什么风格的回答（要点式/先结论/详细解释…）？——把回答**写进 `system/CLAUDE.md` 的"你的主人"一节**，并复述一遍确认。
